...
# a script rather than a grep in each: the release workflow refuses to publish a
# tag whose version has no changelog entry, and the GitHub release it then
# creates uses that entry as its notes. A version whose notes are checked in one
# place and rendered in another is a version whose notes drift.
#
# Usage: scripts/changelog-section.sh 0.3.0 [path/to/CHANGELOG.md]
set -euo pipefail
...
# next `## ` heading, or — for the oldest version, which has none after it — the
# block of `[0.1.0]: https://...` link definitions at the foot of the file.
